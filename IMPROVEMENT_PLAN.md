...
| 自动测试 | 27 个测试通过（model / service / storage） |
...
- 测试骨架：新增 `src/test/java`，`mvn verify` 运行 27 个测试全部通过
  （`BookRecordTest` 5、`BookCodecTest` 11、`BookRepositoryTest` 11）。
...
- 仍需人工执行：真实服务器手动回归（场景 1-17）与 1.20.5+ 兼容探索。

## 复查结果（2026-05-24）

- `mvn verify`：通过，27 个测试通过，构建 `target/booklite-0.1.0.jar`。
...
- [x] 至少有核心单元测试，不再显示 `No tests to run`（27 个测试通过）。

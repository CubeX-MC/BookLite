...
public class BookLiteCommand implements CommandExecutor, TabCompleter {
    ...
    private boolean handleStatus(CommandSender sender) {
        ...
        try {
            ...
            p.put("active", String.valueOf(stats.active()));
            p.put("deleted", String.valueOf(stats.deleted()));
    ...
    private boolean handleInfo(CommandSender sender, String[] args) {
        ...
        try {
            ...
            p.put("status", record.isDeleted() ? lang.msg("admin.status_deleted") : lang.msg("admin.status_active"));
            p.put("created", Instant.ofEpochMilli(record.createdAt()).toString());
            p.put("updated", Instant.ofEpochMilli(record.updatedAt()).toString());
            p.put("deleted_at", record.deletedAt() == null ? "-" : Instant.ofEpochMilli(record.deletedAt()).toString());
            ...
            sender.sendMessage(lang.msg("admin.info_status", p));
    ...
    private boolean handleDelete(CommandSender sender, String[] args, boolean delete) {
        if (!sender.hasPermission("booklite.admin.delete")) {
            lang.send(sender, "commands.no_permission");
            return true;
        }
        ...
        try {
            ...
            boolean changed = delete ? books.delete(record.id()) : books.undelete(record.id());
            lang.send(sender, changed
                    ? (delete ? "admin.deleted" : "admin.undeleted")
                    : "admin.delete_no_change");
        } catch (SQLException ex) {
            books.logStorageFailure(delete ? "delete" : "undelete", ex);
            lang.send(sender, "book.fail_storage");
        }
        return true;
    }
